...
To scrape stuff from the web, you need to look "under the hood" at the site code by right-clicking the element you are interested in and selecting "inspect". I had to first ID the "xpath" of the information you are interested in targeting and look for them in the site source code.

This page has a table with each cruise ship, and the table contains a link to that ship's own Wikipedia page, where the information I wanted (capacity) could be found. 
...
**Xpaths for the capacity element on each ship's page:**
...
#create a few dfs to fil in any last missing info 
...
# read in the csv so the loop above doesnt have to run everytime (not included)
cruise_capacities_final <- read_csv("outputs/cruise_capacities_final.csv") %>% 
  mutate(
    capacity_n = case_when(
    shipname == "CELEBRITY EQUINOX" ~ 2850,
    shipname == "INDEPENDENCE OF SEAS" ~ 4375, #maximum berths, because capacity not listed
    TRUE ~ capacity_n),
    crew_n = case_when(
           shipname == "CELEBRITY EQUINOX" ~ 1250,
           shipname == "INDEPENDENCE OF SEAS" ~ 0, # could not confirm crew
           TRUE ~ crew_n)
         ) %>% 
  mutate(total_ship_pop = capacity_n + crew_n)
